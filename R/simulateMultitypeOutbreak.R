...
#' @param host.type.probs Vector of probabilities of the different host types. Defaults to one type.
...
#' @export
simulateMultitypeOutbreak = function(host.type.probs = c(1), off.r=1,off.p=c(0.5),neg=0.25,nSampled=NA,pi=0.5,w.shape=2,w.scale=1,ws.shape=NA,ws.scale=NA,w.mean=NA,w.std=NA,ws.mean=NA,ws.std=NA,dateStartOutbreak=2000,dateT=Inf) {
  ...
  while (is.na(nSampled)||nsam!=nSampled) {
    ttree=NULL
    while (is.null(ttree)) {
      mtt<-makeMultitypeTTree(host.type.probs,off.r,off.p,pi,w.shape,w.scale,ws.shape,ws.scale,dateT-dateStartOutbreak,nSampled)
      rejected=rejected+1
      ttree<-mtt$ttree
      probttree<-mtt$prob
      if (mtt$pruned>0) {
...
# test
set.seed(4)
outbreak <- simulateMultitypeOutbreak(host.type.probs = c(0.5,0.5), off.r=1, off.p=c(0.5, 0.3), pi=0.5, w.shape=2, w.scale=1, dateStartOutbreak = 0)
outbreak$ctree
